...
class SpeedReaderApp:
  def __init__(self, root: tk.Tk) -> None:
    ...
    self.frame: ttk.Frame = ttk.Frame(root, padding=10)
    self.frame.pack(fill="both", expand=True)

    self.label: ttk.Label = ttk.Label(self.frame, text="", wraplength=800, anchor="center")
    self.label.pack(pady=20)

    self.start_button: ttk.Button = ttk.Button(self.frame, text="Start", command=self.start)
    self.start_button.pack(pady=5)

    self.stop_button: ttk.Button = ttk.Button(self.frame, text="Stop", command=self.stop)
    self.stop_button.pack(pady=5)

    self.open_button: ttk.Button = ttk.Button(self.frame, text="Open Text File", command=self.open_file)
    self.open_button.pack(pady=5)

    self.speed_label: ttk.Label = ttk.Label(self.frame, text="Speed: 300 ms per word", font=("Helvetica", 14))
    self.speed_label.pack(pady=5)

    self.speed_scale: ttk.Scale = ttk.Scale(self.frame, from_=100, to=1000, orient="horizontal", length=650, command=self.update_speed_label)
    self.speed_scale.pack(pady=5)

    # Add incremental labels
    self.increment_frame: ttk.Frame = ttk.Frame(self.frame, width=750, height=30)
    self.increment_frame.pack(pady=5)

    # Evenly spaced speed increments
    increments: List[int] = [100, 400, 700, 1000]
    positions: List[float] = [0.08, 0.36, 0.64, 0.9] # Relative positions based on length

    for increment, position in zip(increments, positions):
      button: ttk.Button = ttk.Button(self.increment_frame, text=f"{increment} ms", command=lambda inc=increment: self.set_speed(inc), style="Increment.TButton")
    ...
    self.wpm_label: ttk.Label = ttk.Label(self.frame, text=" words per minute", font=("Helvetica", 14), background="#1A1A2E", foreground="white")
    self.wpm_label.pack(pady=(30, 15))

    # Display reading time estimate for 300 page book @ 300 words per page
    self.book_time_label: ttk.Label = ttk.Label(self.frame, text="Estimated time to read a 300-page book: ", font=("Helvetica", 14), background="#1A1A2E", foreground="white")
    self.book_time_label.pack(pady=(15, 0))

    self.time_estimate_explainer: ttk.Label = ttk.Label(self.frame, text="Assuming an average of 300 words per page", font=("Helvetica", 10), background="#1A1A2E", foreground="#3A506B")
    self.time_estimate_explainer.pack(pady=0)

    self.text: str = self.load_default_text()
  ...
  def load_default_text(self) -> str:
    try:
  ...
  def open_file(self) -> None:
    file_path: Optional[str] = filedialog.askopenfilename(filetypes=[("Text files", "*.txt")])
    if file_path:
  ...
  def update_speed_label(self, value: str) -> None:
    self.speed_label.config(text=f"Speed: {int(float(value))} ms per word")
    self.update_wpm_and_book_time(int(float(value)))

  def update_wpm_and_book_time(self, ms_per_word: int) -> None:
    words_per_minute: int = 60000 // ms_per_word
    self.wpm_label.config(text=f"{words_per_minute} words per minute")
    minutes_to_read_book: int = 90000 // words_per_minute
    hours: int = minutes_to_read_book // 60
    minutes: int = minutes_to_read_book % 60
...
if __name__ == "__main__":
  root: tk.Tk = tk.Tk()
  app: SpeedReaderApp = SpeedReaderApp(root)
  root.mainloop()